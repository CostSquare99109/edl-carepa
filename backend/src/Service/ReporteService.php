...
class ReporteService
{
 ...
 $pdo = Database::getInstance();
	...
	ev.primer_nombre as evaluador_nombres, ev.primer_apellido as evaluador_apellidos,
 ...
 if (empty($detalles)) {
	...
	FROM compromisos co
	WHERE co.concertacion_id IN (SELECT concertacion_id FROM evaluaciones WHERE id = ?) AND co.eliminado_en IS NULL
	");
 $stmt->execute([$id]);
 $detalles = $stmt->fetchAll();
 }
