      ...
      <table className="edl-table" aria-label={ariaLabel}>
            ...
            {selectable ? (
              <th scope="col" className="w-10">
                <input
                  ...
                  onChange={toggleAll}
                  className="rounded border-inst-borde text-inst-azul focus:ring-inst-azul"
                />
              </th>
            ) : null}
            {columns.map((c) => {
                ...
                >
                  {c.sortable && onSortChange && c.sortKey ? (
                    <button
                      ...
                      className={[
                        'inline-flex items-center gap-1 font-semibold hover:text-inst-azul transition-colors',
        ...
        <tbody>
          {data.map((row) => {
            ...
            return (
              <tr key={id} className={isSelected ? 'bg-inst-azul-light/40' : ''}>
                {selectable ? (
                  <td>
                    <input
                      type="checkbox"
