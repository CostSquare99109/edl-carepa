...
export default function AdminUsuarios() {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      setUsuarios(res.data || res.items || []);
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b bg-inst-gris">
              ...
              <th className="text-left px-4 py-3 font-semibold text-inst-texto-claro">Roles</th>
                ...
                <tr key={u.id} className="border-b hover:bg-inst-gris/50 transition">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1 flex-wrap">
                      {(u.roles || []).map(r => (
                        <span key={r.codigo} className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border ${ROLE_COLORS[r.codigo] || 'bg-gray-100 text-gray-700'}`}>
                          {r.nombre}
                        </span>
                      ))}
                    </div>
